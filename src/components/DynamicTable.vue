<!-- src/components/pokedex/DynamicTable.vue -->
<template>
  <div class="table-wrap">
    <Table>
      ...
      <TableBody v-if="(!loading && rows.length) || pushing">
        <TableRow
            v-for="row in rows"
            :key="row.id"
            :class="isSelected(row.id) ? 'bg-muted/40' : ''"
      ...
      <TableBody v-else>
        <TableRow>
          <TableCell :colspan="colspan" class="p-6 text-center text-sm text-foreground/60">
            No items.
          </TableCell>
        </TableRow>
      </TableBody>
    ...
    <div v-if="pagination" class="flex items-center justify-between gap-3 p-3 sticky bottom-10">
      ...
      >
        <PaginationContent v-slot="{ items }">
          ...
          <template v-for="(item, i) in items" :key="i">
            ...
            <!-- Ellipsis items (clicking does nothing) -->
...
export type DynamicColumn = {
  key: string      // accessor in row (e.g., 'image', 'name', 'caughtAt')
  label: string
  class?: string
  cellClass?: string
}

export type DynamicRow = Record<string, any> & { id: number }
...
const allChecked = computed(() => rowIds.value.length > 0 && rowIds.value.every(id => selectedSet.value.has(id)))
const someChecked = computed(() => rowIds.value.some(id => selectedSet.value.has(id)))

const colspan = computed(() => (selecting.value ? 1 : 0) + (props.columns?.length ?? 0))
...
const end = computed(() => Math.min(start.value + props.rows.length, total.value))
...
function onRowClick(data: DynamicRow) {
  emit('row-click', data);
}
